...
textInput("artist_name","Search by artist:", value = "Khalid",width = "200px", placeholder = "Khalid")
...
g + geom_violin() + 
  labs(title="Music Characterstics", 
       subtitle="How particular musical attributes vary with the top artists?",
       x="Artist",
       y=input$y)
...
search_by_artist <- reactive({
  billboard_subset()[like(artist,input$artist)]
})

output$DataTable2 <- DT::renderDataTable(
    DT::datatable(data = search_by_artist(), 
                  options = list(pageLength = 50, scrollY="300px"), 
                  rownames = FALSE)
)
